...
export function defaultEventSessionSender(
  ...
  errorCallback: (reason: string) => void = nop): (sessionEvents: SessionEvent[]) => void {
  return (sessionEvents: SessionEvent[]) => sendSessionEvents(authKey, sessionEvents, successCallback, errorCallback)
}
